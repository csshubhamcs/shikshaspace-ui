...
/** REST client to communicate with User Service Handles login, register, profile operations */
@Slf4j
@Service
public class UserServiceClient {
  ...
  public Mono<AuthResponse> register(RegisterRequest request) {
    ...
    return webClient
        ...
        .timeout(Duration.ofMillis(timeout))
        .doOnSuccess(
            response -> log.info("Registration successful for user: {}", request.getUsername()))
        .doOnError(
            error -> log.error("Registration failed for user: {}", request.getUsername(), error));
  }
  ...
  public Mono<UserResponse> updateProfile(
      UUID userId, UpdateProfileRequest request, String jwtToken) {
